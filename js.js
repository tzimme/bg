...
var feld=new Array(28),
	...
	line=new Array(12),
	voll=new Array(3),
	w1,w2,dran,selected,wuerfeln,pasch,i,l,ll,t,m1,m2,start,possible,min,max,temp,geschlagen,geht,zuege;
			...
			var $oppName="";
				...
				socket.on('names',function(data){
					...
					for(i=0;i<data.length;i++){
						if(data[i].localeCompare($playerName)==0||data[i]==$playerName){
							playerDivId='id="me" name="'+escapeHtml($playerName)+'"';
						}else{
							playerDivId="id=\""+escapeHtml(data[i])+"\"";
							if(data[i]==$oppName||data[i].localeCompare($oppName)==0){
								stopGame=0;
							}
						}
						htm +="<div class=\"player\" "+playerDivId+">"+escapeHtml(data[i])+"</div>";
					}
					if(stopGame==1){
						$('#backgammon').hide();
						$('#lobby').show();		
				...
				socket.on('invite',function(data){
					...
					d.className=d.className+" inviting";
					invites.push(data);
				});
				...
				$(document).on('click','.inviting',function(){
					var match={
						p1:$(this).text(),
						p2:$playerName
					};
				...
				$(document).on('click','.player',function(){				
					if($(this).attr('id').localeCompare('me')!=0&&$(this).attr('id')!="me"&&$(this).attr('class').localeCompare('invited')!=0&&$(this).attr('class')!="invited"){
						$(this).attr('class',' player invited');
						var invitation={
							from:$playerName,
							to:$(this).text()
						};
				...
				$(document).on('click','.invited',function(){
					...
					socket.emit('cancel',cancelation);
				});
...
$(document).on('click', 'font',function(){
    ...
    if(auswahl!="wuerfel"&&wuerfeln!=1){
        ...
        if(selected==''){
            ...
            if(!geschlagen&&auswahl>0&&auswahl<29){
...
function checkMove(position){
    geht=false;
    if(position>0&&position<25&&feld[27+dran][1]==0){
        if(w1>=1&&w1<=6){
...
function draw(){
	...
	for(l=7;l<12;l++){
        for(i=13;i<25;i++){
            ...
            if(i==24&&ll==1&&feld[25][1]>0){
                line[l]+='<font class="f0" color="yellow">'+feld[25][1]+'</font>&nbsp;';
            }
        }
        ll--;
    }
